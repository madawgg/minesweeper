...
export default class Board {
   ...
   */
  constructor(columns, rows, mines, level) {
    this.mines = mines;
    this.level = level;
  ...
  allMinesFlagged = () => {
    return this.mines.every(mine => this.getCellBy(mine.row, mine.column).flagged);
  }
  ...
  _defusedCells = () => {
    ...
    return cells.filter(x => x.defused).length;
  }
  ...
  flag = (row, column) => {
    ...
    if (this.mines.length === this.flaggedCount && !cell.flagged || cell.defused || cell.exploded) return this;
  ...
  /**
     * Gets all cells in the board.
     * @returns {Array} Array of all cells in the board.
     */
